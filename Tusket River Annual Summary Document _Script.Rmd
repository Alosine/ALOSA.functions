...
Year\| Total number of gaspereau ascending the ladder\| Number of Alewife\| Number of Blueback Herring\| Exploitation rate estimate for Alewife\| Exploitation rate estimate for Blueback Herring ------- \| ----------------\| ----------------\| ----------------\| --------\| -------- 2021\| 1,956,804\| 1,941,411\| 15,394\| 0.38^f^\| 0.44^f^ 2020^d^\| NA\| NA\| NA\| NA\| NA 2019\| 397,709\| 367,781\| 29,928\| 0.51^e^\| 0.49^e^ 2018^a^\| 712,509\| NA\| NA\| NA\| NA 2015\| 2,517,215\| 2,325,787\| 191,428\| 0.52^c^\| 0.35^c^ 2014^b^\| 2,358,125\| 1,767,745\| 590,380\| 0.60^c^\| 0.38^c^
...
multiyear=read.csv("//ent.dfo-mpo.ca/atlshares/Science/Population Ecology Division/DFD/Alosa/Assessment Results/river summaries all years tusket update.csv",
...
plotyears=c(2014,2015,2018,2019)
#windows(8.5,11)
par(oma=c(7,3,3,0.5),mfrow=c(4,1))
par(mar=c(0,5,0,3))

for(plotyear in plotyears){
plot(1,1, pch=" ",xlab="", ylab="",ylim=c(0,(max(TRdata$number[TRdata$year==plotyear])*1.2/1000)),xlim=c(96,179),
     xaxt="n",yaxt='n',cex.axis=2)
rect(91, -5, 120, (max(TRdata$number[TRdata$year==plotyear])*1.2/1000)+5, border = NA,col="gray90")
rect(152, -5, 181, (max(TRdata$number[TRdata$year==plotyear])*1.2/1000)+5, border = NA,col="gray90")
box()
...
**Figure 4.** Estimated number of Alewife and Blueback Herring ascending the Lake Vaughan fish ladder in 2014, 2015, 2018, and 2019. In 2018 no species identification was done as the decision to count video was made after the spawning run was over, so counts are not split by the two species. Note that scales differ among plots.
...
if(length(TRdata$number[TRdata$year==plotyear])>0){
 plot(1,1, pch=" ",xlab="", ylab="",ylim=c(0,(max(TRdata$number[TRdata$year==plotyear])*1.2/1000)),xlim=c(91,179),
     xaxt="n",yaxt='n',cex.axis=2) 
rect(91, -5, 120, (max(TRdata$number[TRdata$year==plotyear])*1.2/1000)+5, border = NA,col="gray90")
rect(152, -5, 181, (max(TRdata$number[TRdata$year==plotyear])*1.2/1000)+5, border = NA,col="gray90")
text(171,max(TRdata$number[TRdata$year==plotyear]/1000),plotyear,cex=2)
text(89,max(TRdata$number[TRdata$year==plotyear]/1000),paste("Total     =", TR$escape[!is.na(TR$year==plotyear)& TR$year==plotyear & TR$species=="All"],sep=" "),cex=1.5,pos=4)
text(89,max(TRdata$number[TRdata$year==plotyear]/1000)*0.8,paste("Total A =", TR$escape[!is.na(TR$year==plotyear)& TR$year==plotyear & TR$species=="A"],sep=" "),cex=1.5,pos=4)
text(89,max(TRdata$number[TRdata$year==plotyear]/1000)*0.6,paste("Total B =", TR$escape[!is.na(TR$year==plotyear)& TR$year==plotyear & TR$species=="B"],sep=" "),cex=1.5,pos=4)
}else{
...
**Figure 5** Estimated number of Alewife and Blueback Herring ascending the Lake Vaughan fish ladder in 2021-2022. No escapement estimates were obtained in 2020 due to the COVID-19 pandemic. Note that scales differ among plots.
