...
for(i in plotyears){
  plotyear=i
plot(1,1, pch=" ",xlab="", ylab="",ylim=c(0,(max(TRdata$number[TRdata$year==plotyear])*1.2/1000)),xlim=c(0,68),
     xaxt="n",yaxt='n',cex.axis=2)
rect(-5, -5, 12, (max(TRdata$number[TRdata$year==plotyear])*1.2/1000)+5, border = NA,col="gray90")
rect(43, -5, 73, (max(TRdata$number[TRdata$year==plotyear])*1.2/1000)+5, border = NA,col="gray90")
box()
lines(TRdata$daynum[TRdata$year==plotyear],TRdata$number[TRdata$year==plotyear]/1000,
      lty=1,lwd=1.5)
text(63,max(TRdata$number[TRdata$year==plotyear]/1000),plotyear,cex=2)
text(-2,max(TRdata$number[TRdata$year==plotyear]/1000),paste("Total =", GR$total[!is.na(GR$year==plotyear)& GR$year==plotyear],sep=" "),cex=1.5,pos=4)
