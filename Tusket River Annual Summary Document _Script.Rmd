...
header-includes:
   ...
   - \fancyhead[L]{Tusket River 2023 Assessment Summary}
   - \fancyfoot[R]{\thepage}
...
```{r hardcode_escapement, include=FALSE, eval = FALSE}
#2015
temp <- read_csv("R:/Science/Population Ecology Division/DFD/Alosa/Locations/Tusket River/Tusket 2015/Counts/Combined_final_counts.csv")

# We need to clean up the data from this year so that the escapement function
# will run
temp <- temp |> 
  select(-1, -study.day) |> 
  rename(count.upstream = count)

temp$year <- 2015
temp$count.downstream <- 0

temp <- temp |> relocate(year)

write_csv(temp, "R:/Science/Population Ecology Division/DFD/Alosa/Locations/Tusket River/Tusket 2015/Counts/counts_for_2024_assessment.csv")

temp <- onespecies.partial.river.escapement(
  filename = "R:/Science/Population Ecology Division/DFD/Alosa/Locations/Tusket River/Tusket 2015/Counts/counts_for_2024_assessment.csv",
  database = FALSE,
  fixtime = FALSE,
  year = 2015,
  site = 2,
...
total_2015 <- sum(temp$total)
clow_2015 <- sum(temp$clow)
chigh_2015 <- sum(temp$chigh)
ci_2015 <- chigh_2015 - clow_2015
...
#   channel = channel)
# 
# total_2019 <- sum(temp$total)
# clow_2019 <- sum(temp$clow)
# chigh_2019 <- sum(temp$chigh)
# ci_2019 <- chigh_2019 - clow_2019
...
#   channel = channel)
# 
# total_2021 <- sum(temp$total)
# clow_2021 <- sum(temp$clow)
# chigh_2021 <- sum(temp$chigh)
# ci_2021 <- chigh_2021 - clow_2021
...
total_2022 <- sum(temp$total)
clow_2022 <- sum(temp$clow)
chigh_2022 <- sum(temp$chigh)
ci_2022 <- chigh_2022 - clow_2022
...
#   channel = channel)
# 
# total_2023 <- sum(temp$total)
# clow_2023 <- sum(temp$clow)
# chigh_2023 <- sum(temp$chigh)
# 
# temp <- onespecies.river.escapement(
...
#   )
# 
# total_2023 <- total_2023 + sum(temp$total)
# clow_2023 <- clow_2023 + sum(temp$clow)
# chigh_2023 <- chigh_2023 + sum(temp$chigh)
# ci_2023 <- chigh_2023 - clow_2023
...
# These are the confidence intervals for the total escapement that we should add
# to the daily escapement plots. The CI were obtained by running the script
# onspecies.river.escapement, taking the CI and dividing it by two since we are
# going to be displaying this as a +/- value instead of a range. For years
# >= 2022, the CI includes that from the Powerhouse data as well
TR2014 <- ""; TRCI2015 <- ""
TR2015 <- as.character(round(total_2015)); TRCI2014 <- as.character(round(ci_2015 / 2))
TR2019 <- as.character(round(total_2019)); TRCI2019 <- as.character(round(ci_2019 / 2))
TR2021 <- as.character(round(total_2021)); TRCI2021 <- as.character(round(ci_2021 / 2))
TR2022 <- ""; TRCI2022 <- ""
TR2023 <- ""; TRCI2023 <- ""
TR2024 <- "2142665"; TRCI2024 <- "66084.5"
```
...
-----|---------|----------|----------|--------|--------|
2024| 2,261,465| 2,249,947| 11,518| 0.34| 0.14
...
**Figure 5.** Estimated number of Alewife and Blueback Herring ascending the Lake Vaughan fish ladder in 2023 and 2024. The solid line represents the total daily count, the dotted line represents the daily Alewife count, and the dotted line represents the daily Blueback Herring Count. Note that scales differ among plots.
