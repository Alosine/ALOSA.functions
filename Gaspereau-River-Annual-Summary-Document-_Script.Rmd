...
- \fancyhead[L]{Gaspereau River 2024 Assessment Summary}
- \fancyfoot[R]{\thepage}
...
\newpage
Please note that all 2024 values related to catch are interim as of October 21 2024 while an updated 2024 catch estimate is being determined.
```{r hardcode_escapement, include=FALSE}
GR2024<-"588,540"; GRCI2024<-"31,210"
GR2023<-"1,312,700"; GRCI2023<-"52,246"
GR2022<-"874,862"; GRCI2022<-"36,935"
GR2021<-"984,094"; GRCI2021<-"36,504"
GR2019<-"1,021,186"; GRCI2019<-"41,881"
GR2018<-"1,061,688"; GRCI2018<-"43,546"
GR2017<-"1,114,450"; GRCI2017<-"40,127"
GR2016<-"454,800"; GRCI2016<-"25,221"
GR2015<-"438,874"; GRCI2015<-"24,070"
#no CI available for 2013 due to method used
```

**Table 1.** Summary of catch data, estimated escapement, estimated run size, and exploitation rate for Alewife on the Gaspereau River, NS. Italicized values for 2024 are placeholders while an updated 2024 catch estimate is being determined.

Year | Catch estimate| Escapement estimate| Estimated run size| Exploitation rate (%)
------- | -------------| ------------------| -------------| --------
2024| 1,265,348| `r GR2024`+/-`r GRCI2024` |1,853,888 |68.2
2023| 1,431,500| `r GR2023`+/-`r GRCI2023` |2,744,200 |52.2
2022| 1,562,900| `r GR2022`+/-`r GRCI2022` |2,437,762 |64.1
2021| 1,231,005| `r GR2021`+/-`r GRCI2021` |2,215,099 |55.6
2020| 1,202,604| NA| NA| NA
2019|	784,152|	`r GR2019`+/-`r GRCI2019`	|1,805,338	|43.4
2018|	903,655|	`r GR2018`+/-`r GRCI2018`	|1,965,343	|46.0
2017*|	605,900|	`r GR2017`+/-`r GRCI2017`	|1,720,350	|35.2
2016*|	769,133|	`r GR2016`+/-`r GRCI2016`	|1,223,933	|62.8
2015*|	705,500|	`r GR2015`+/-`r GRCI2015`	|1,144,374	|61.7
...
**Notes:** Data from 1970 through 2006 are from McIntyre et al. (2007). Other data are unpublished. Escapement estimates from 2003 to 2024 obtained by sub-sampling video. In 2013, the escapement estimate is number of fish estimated between 0600h and 2000h adjusted by the proportion of fish that moved between 0000h and 0600h and between 2000h and 2400h from 2015-2019. Total counts were used in other years. The catch estimates for 2012 to 2024 were provided by the buyer initially as the number of pounds purchased. These are converted to the number of fish by dividing the number of pounds purchased by the weight per pail (30 lbs), and multiplying by the estimated number of fish per pail (83 fish/pail). The 2012 to 2017 values are not adjusted for landings that were sold as bait or for local consumption. The 2018, 2019, and 2021 catch estimates are adjusted for fish sold as bait. The count in 1995 is a partial count but is considered mostly complete. Values may change as further information about the fishery is obtained. No escapement estimates were calculated in 2020 due to the COVID-19 pandemic.  
NA = not available  
...
| | | 
Exploitation Rate Removal Reference Level|	µRR|	Exploitation rate above which the stock is over exploited|	0.53
| | | 
Exploitation Rate Lower Removal Reference Level|	µLRR|	Exploitation rate above which stock is fully exploited, below which stock is under exploited|	0.35
...
reference.point.plot<-function(ssnmsy,
  ...
  mtext(paste("µ RR =",Fupper,sep=""),1,line=4,at=(max(pointsX,na.rm=T)*1.1),las=0,adj=1)
  mtext(paste("µ LRR =",Ffully,sep=""),1,line=5,at=(max(pointsX,na.rm=T)*1.1),las=0,adj=1)
...
text(0.98,1.1,"2015",pos=4,col="gray35",cex=0.85)
text(1.1,1.21,"2016",pos=4,col="gray35",cex=0.85)
...
text(2.3,0.97,"2021",pos=3,col="gray35",cex=0.85)
text(2.35,1.2,"2022",pos=3,col="gray35",cex=0.85)
text(3.4,0.98,"2023",pos=3,col="gray35",cex=0.85)
text(1.3,1.25,"2024",pos=3,col="red",cex=0.85)
arrows(0.65,1.45,0.43,1.37,length=0.1,col="gray35",cex=0.85)
```

**Figure 3.** Stock status plot for the Gaspereau River Alewife stock. Each point indicates status in a single year, and the lines connect the points in sequence. Stock status from 1982-1984 are show using open circles, status from 1997-2006 are shown using an X, and for 2013-19 and 2021-2024 using closed circles. The plot is updated from Gibson et al. 2017.
...
plotyears=c(2013,2014,2015,2016)
ymax<-max(GRdata$number[GRdata$year %in% plotyears])/1000
...
for(i in plotyears){
  plotyear=i
plot(1,1, pch=" ",xlab="", ylab="",ylim=c(0,(ymax*1.2)),xlim=c(0,68),
     xaxt="n",yaxt='n',cex.axis=2)
rect(-5, -5, 12, (ymax*1.2)+5, border = NA,col="gray90")
rect(43, -5, 73, (ymax*1.2)+5, border = NA,col="gray90")
box()
lines(GRdata$daynum[GRdata$year==plotyear],GRdata$number[GRdata$year==plotyear]/1000,
      lty=1,lwd=1.5)
text(63,ymax*1.05,plotyear,cex=2)
if(plotyear==2013){text(-2,ymax*1.05,paste("Total = ","149,682", sep=""),cex=1.5,pos=4)}
if(plotyear==2014){text(-2,ymax*1.05,"No Count",cex=1.5,pos=4)}
if(plotyear==2015){text(-2,ymax*1.05,paste("Total = ",paste(GR2015,GRCI2015,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2016){text(-2,ymax*1.05,paste("Total = ",paste(GR2016,GRCI2016,sep=" ± "), sep=""),cex=1.5,pos=4)}
...
**Figure 4.** Estimated number of Alewife ascending the White Rock fish ladder in 2013-2016.

\newpage
```{r dailyplot2,echo=FALSE, fig.width = 7.5,fig.height = 9}
plotyears=c(2017,2018,2019,2020)
ymax<-max(GRdata$number[GRdata$year %in% plotyears])/1000
...
rect(43, -5, 73, (ymax*1.2)+5, border = NA,col="gray90")
text(63,ymax*1.05,plotyear,cex=2)
if(plotyear==2017){text(-2,ymax*1.05,paste("Total = ",paste(GR2017,GRCI2017,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2018){text(-2,ymax*1.05,paste("Total = ",paste(GR2018,GRCI2018,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2019){text(-2,ymax*1.05,paste("Total = ",paste(GR2019,GRCI2019,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2020){text(-2,ymax*1.05,"No Count",cex=1.5,pos=4)}
}else{
...
**Figure 5.** Estimated number of Alewife ascending the White Rock fish ladder in 2017-2020.

\newpage
```{r dailyplot3,echo=FALSE, fig.width = 7.5,fig.height = 9}
plotyears=c(2021,2022,2023,2024)
ymax<-max(GRdata$number[GRdata$year %in% plotyears])/1000
...
rect(43, -5, 73, (ymax*1.2)+5, border = NA,col="gray90")
text(63,ymax*1.05,plotyear,cex=2)
if(plotyear==2021){text(-2,ymax*1.05,paste("Total = ",paste(GR2021,GRCI2021,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2022){text(-2,ymax*1.05,paste("Total = ",paste(GR2022,GRCI2022,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2023){text(-2,ymax*1.05,paste("Total = ",paste(GR2023,GRCI2023,sep=" ± "), sep=""),cex=1.5,pos=4)}
if(plotyear==2024){text(-2,ymax*1.05,paste("Total = ",paste(GR2024,GRCI2024,sep=" ± "), sep=""),cex=1.5,pos=4)}
}else{
 plot(1,1, pch=" ",xlab="", ylab="",ylim=c(0,50),xlim=c(0,68),
       xaxt="n",yaxt='n',cex.axis=2)
  rect(-5, -5, 12, 55, border = NA,col="gray90")
...
**Figure 8.** Estimated proportion of Alewife by age and spawning history ascending the White Rock fish ladder in 2016 and 2018.
...
**Figure 9.** Estimated proportion of Alewife by age and spawning history ascending the White Rock fish ladder in 2019 and 2021. 

\newpage
```{r ageplot3,echo=FALSE, fig.width = 7.5,fig.height = 9}
plotyears=c(2022,2023)
...
**Figure 10.** Estimated proportion of Alewife by age and spawning history ascending the White Rock fish ladder in 2022 and 2023.

\newpage
<!-- ![](//ent.dfo-mpo.ca/atlshares/Science/Population Ecology Division/DFD/Alosa/Assessment Results/Figure9.png)  -->
![](C:/~/git/ALOSA.functions/figure pngs/Figure9.png)


**Figure 11.** Yield curve showing the relationship between the exploitation rate (the proportion of the spawning run harvested) and the yield from the fishery. The three zones correspond to the removal reference levels provided in Table 2. Adapted from Gibson et al. (2017).
